    ...
    public partial class Cart : System.Web.UI.Page
    {
            ...
            if(e.CommandName == "updateCart")
            {
                ...
                for(int item =0; item < rCartItem.Items.Count; item++)
                {
                    if (rCartItem.Items[item].ItemType == ListItemType.Item || rCartItem.Items[item].ItemType == ListItemType.AlternatingItem)
                    {
                        ...
                        if (isTrue)
                        {
                            // Update cart item's quantity inDB
            ...
            if(e.CommandName == "checkout")
            {
                ...
                for (int item = 0; item < rCartItem.Items.Count; item++)
                {
                    if (rCartItem.Items[item].ItemType == ListItemType.Item || rCartItem.Items[item].ItemType == ListItemType.AlternatingItem)
                    {
                        ...
                        int productId = Convert.ToInt32(_productId.Value);
                        int cartQuanity = Convert.ToInt32(_cartQuantity.Value);
                        int productQuantity = Convert.ToInt32(_productQuantity.Value);


                        if (productQuantity > cartQuanity && productQuantity > 2)
                        {
                            isTrue = true;
                        }
                        else {
        ...
        private sealed class CustomTemplate : ITemplate
        {
            ...
            public void InstantiateIn(Control container)
            {
                ...
                {
                    var footer = new LiteralControl("<tr><td colspan='5'><b>Your Cart is empty.</b><a href='Menu.aspx' " +
                        "class='badge badge-info ml-2'>Continue Shopping</a></td></tr></tbody></table>");
